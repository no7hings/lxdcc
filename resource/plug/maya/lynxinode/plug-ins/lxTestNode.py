    ...
    def update_drag_information(self):
        ...
        if active:
            ...
            if (active):
                ...
                min_change_value = min(self.mouse_point_gl_name.x, self.mouse_point_gl_name.y,
                                       self.mouse_point_gl_name.z)
                max_change_value = max(self.mouse_point_gl_name.x, self.mouse_point_gl_name.y,
                                       self.mouse_point_gl_name.z)
                if (active == self.right_name):
                ...
                while not iter.isDone():
                    ...
                    if (not manipulator.connectToDependNode(depend_node)):
                        OpenMaya.MGlobal.displayWarning(
                            'Error connecting manipulator to object %s' % depend_node_fn.name())
                        next(iter)
                        continue
