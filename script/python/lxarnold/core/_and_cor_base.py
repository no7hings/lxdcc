...
class AndPortMtd(object):
    ...
    def _get_constant_raw_(self):
        ...
        if and_type in and_configure.AndCfgUtil.AR_VALUE_FNC_DICT:
            fnc = and_configure.AndCfgUtil.AR_VALUE_FNC_DICT[and_type]
            if fnc is not None:
    ...
    def _get_array_raw_(self):
        ...
        if and_exact_type in and_configure.AndCfgUtil.AR_ARRAY_VALUE_FNC_DICT:
            lis = []
            fnc = and_configure.AndCfgUtil.AR_ARRAY_VALUE_FNC_DICT[and_exact_type]
    ...
    def _get_default_constant_raw_(self):
        and_type = self.type
        and_port = self.and_instance
        if and_type in and_configure.AndCfgUtil.AR_DEFAULT_VALUE_FNC_DICT:
            fnc = and_configure.AndCfgUtil.AR_DEFAULT_VALUE_FNC_DICT[and_type]
            raw = fnc(and_port)
            return self._set_raw_convert_to_dcc_style_(and_type, and_port, raw)
