...
class _AbsFileReaderOld(object):
    SEP = '\n'
...
class DotAssReaderOld(_AbsFileReaderOld):
    def __init__(self, file_path):
        super(DotAssReaderOld, self).__init__(file_path)
...
class DotXgenFileReader(_AbsFileReaderOld):
    SEP = '\n\n'
    FILE_REFERENCE_DICT = {
    ...
    def _get_obj_raws_(self):
        ...
        pattern_0 = _ReadPatternOld(u'{obj_type}\n{port_lines}\n')
        lines = fnmatch.filter(
...
class DotMaFileReaderOld(_AbsFileReaderOld):
    SEP = ';\n'
    FILE_REFERENCE_DICT = {
        'file': 'ftn',
        'reference': 'fn[0]',
    ...
    def __init__(self, file_path):
        super(DotMaFileReaderOld, self).__init__(file_path)

    def _get_references_(self):
        lis = []
        pattern = _ReadPatternOld(u'file -rdi {a} -ns "{namespace}" -rfn "{obj}"{b}-typ{c}"{file_type}"{d}"{file}"{r}')
        lines = fnmatch.filter(
    ...
    def _get_obj_raws_(self):
        lis = []
        pattern_0 = _ReadPatternOld(u'createNode {obj_type} -n "{obj_name}"{r}')
        lines = fnmatch.filter(
            self.lines, pattern_0.pattern
        )
        for i_line in lines:
    ...
    def _get_is_port_(cls, line):
        pattern_0 = _ReadPatternOld(u'{l}setAttr{r}')
        ...
        if results:
            return True
        #
        pattern_1 = _ReadPatternOld(u'{l}addAttr{r}')
        results = fnmatch.filter(
    ...
    def _get_obj_port_raw_(cls, line):
        pattern_0 = _ReadPatternOld(u'{l}setAttr ".{port_name}" -type "{port_type}"{m}"{port_raw}";{r}')
        results = fnmatch.filter(
            [line], pattern_0.pattern
        )
        if results:
            ...
            if p:
                variant = p.named
                return variant
        pattern_1 = _ReadPatternOld(u'{l}setAttr ".{port_name}" {port_raw};{r}')
        results = fnmatch.filter(
            [line], pattern_1.pattern
        )
        if results:
            ...
            if p:
                ...
                return variant
        pattern_2 = _ReadPatternOld(u'{l}addAttr {m0} -ln "{port_name}" -dt "{port_type}";{r}')
        results = fnmatch.filter(
    ...
    def _get_obj_uuid_raw_(cls, line):
        pattern = _ReadPatternOld(u'{l}rename -uuid "{raw}"{r}')
        results = fnmatch.filter(
...
class DotMtlxFileReaderOld(_AbsFileReaderOld):
    SEP = '\n'
    LINE_MATCHER_CLS = _ReadPatternOld
    PROPERTIES_CLS = ctt_core.Properties

    def __init__(self, file_path):
        super(DotMtlxFileReaderOld, self).__init__(file_path)
...
class DotOslFileReaderOld(_AbsFileReaderOld):
    SEP = '\n'
    LINE_MATCHER_CLS = _ReadPatternOld
    PROPERTIES_CLS = ctt_core.Properties

    def __init__(self, file_path):
        super(DotOslFileReaderOld, self).__init__(file_path)
