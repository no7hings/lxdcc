...
from ..core import base as bsc_ddl_cor_base
...
class DdlSubmiter(AbsDdlSubmiter):
    CON = bsc_ddl_cor_base.DdlBase.generate_connection()
    CONFIGURE_FILE_PATH = rsc_core.ResourceConfigure.get_yaml('session/deadline/submiter')
...
class DdlSubmiterForRsvProject(AbsDdlSubmiter):
    CON = bsc_ddl_cor_base.DdlBase.generate_connection()
    CONFIGURE_FILE_PATH = rsc_core.ResourceConfigure.get_yaml('session/deadline/rsv-project-submiter')
...
class DdlSubmiterForRsvTask(AbsDdlSubmiter):
    CON = bsc_ddl_cor_base.DdlBase.generate_connection()
    CONFIGURE_FILE_PATH = rsc_core.ResourceConfigure.get_yaml('session/deadline/rsv-task-submiter')
