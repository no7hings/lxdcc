...
from ..core import _ctt_cor_base
...
class AbsContent(object):
    ...
    def __unfold_quote(self, key_path, keys_all):
        ...
        filter_fnc = _ctt_cor_base.ContentUtil.filter
    ...
    def __unfold_inherit_and_override(self, key_path, keys_all):
        ...
        if filter_fnc([key_path], '*$'):
            value = self.get(key_path)
            if filter_fnc([value], '.*'):
                key_inherit = _ctt_cor_base.ContentUtil.get_absolute_key(value, key_path)
            else:
                key_inherit = value
            #
    ...
    def __init__(self, key=None, value=None):
        ...
        if isinstance(value, six.string_types):
            ...
            if os.path.isfile(file_path):
                ...
                _ = _ctt_cor_base.ContentFile(self.__file_path).read()
    ...
    def save_to(self, file_path):
        _ctt_cor_base.ContentFile(file_path).write(self.__value)

    def save(self):
        _ctt_cor_base.ContentFile(self.__file_path).write(self.__value)
    ...
    def get_keys(self, regex=None):
        ...
        if regex is not None:
            return _ctt_cor_base.ContentUtil.filter(_, regex)
        return _
    ...
    def get_all_leaf_key_as_dag_paths(self):
        ...
        if keys:
            return [_ctt_cor_base.ContentUtil.key_path_to_dag_path(i) for i in keys]
        return []
    ...
    def get_str_as_yaml_style(self):
        return _ctt_cor_base.ContentYamlBase.dump(
            self.value,
            indent=4,
    ...
    def do_flatten(self):
        ...
        for i_key in keys_all:
            i_value = self.get(i_key)
            if isinstance(i_value, dict) is False:
                i_value = _ctt_cor_base.ContentUtil.unfold_fnc(
                    i_key,
                    keys_all=keys_all,
...
class ContentCache(object):
    def __init__(self, file_path):
        ...
        if os.path.isfile(file_path) is False:
            _ctt_cor_base.ContentFile(file_path).write({})
