...
class ScpResourcesAddByQuixel(object):
    ...
    #
    TEXTURE_PATTERN = '{texture_key}_{texture_size_tag}K_{texture_type_tag}.{file_format}'
    TEXTURE_LOD_PATTERN = '{texture_key}_{texture_size_tag}K_{texture_type_tag}_LOD{lod_level}.{file_format}'
    #
    TEXTURE_EXTRA_KEYS = [
        'Atlas',
        'Billboard'
    ...
    #
    GEOMETRY_LOD_PATTERN = '{geometry_key}_LOD{lod_level}.{file_format}'
    ...
    def dtb_create_resource_fnc(cls, dtb_opt, resource_dtb_path, version_dtb_path, resource_directory_path_tgt, json_content):
        ...
        if is_create is True:
            ...
            dtb_opt.create_property(
                resource_dtb_path, 'version', version_dtb_path, kind=dtb_opt.Kinds.Version
            )
            dtb_opt.create_property(
                resource_dtb_path, 'location', resource_directory_path_tgt, kind=dtb_opt.Kinds.Version
            )
            # types
    ...
    def stg_and_dtb_add_textures_extra_fnc(cls, dtb_opt, pattern_kwargs, resource_dtb_path, version_dtb_path, directory_path_src):
        ...
        for i_index, i_key_extra in enumerate(cls.TEXTURE_EXTRA_KEYS):
            ...
            if bsc_core.StgDirectoryOpt(i_directory_path_extra).get_is_exists() is True:
                bsc_core.StgDirectoryOpt(i_directory_path_extra).set_copy_to_directory(
                    '{}/{}'.format(quixel_directory_path_tgt, i_key_extra)
                )
                i_file_paths_src = bsc_core.StgDirectoryOpt(i_directory_path_extra).get_file_paths()
                if i_file_paths_src:
    ...
    def stg_and_dtb_add_geometries_fnc(cls, dtb_opt, pattern_kwargs, resource_dtb_path, version_dtb_path, directory_path_src):
        ...
        for i_file_path_src in file_paths:
            ...
            if i_lod_pattern_opt_src.get_is_matched(i_file_path_src) is True:
                i_file_opt = bsc_core.StgFileOpt(i_file_path_src)
    ...
    @classmethod
    def stg_and_dtb_add_any_geometry_fnc(cls, dtb_opt, pattern_kwargs, pattern_opt_src, resource_dtb_path, version_dtb_path, file_path_src, file_format, keyword, is_lod):
        pattern_opt_tgt = dtb_opt.get_pattern_opt(keyword)
        pattern_kwargs_src = copy.copy(pattern_kwargs)
        variants_src = pattern_opt_src.get_variants(file_path_src)
        pattern_kwargs_src.update(variants_src)
        ...
        for i_var_index in range(20):
            ...
            if bsc_core.StgDirectoryOpt(i_directory_path_extra).get_is_exists() is True:
                ...
                if i_file_paths_src:
                    ...
                    for j_file_path_src in i_file_paths_src:
                        j_file_opt = bsc_core.StgFileOpt(j_file_path_src)
    ...
    @classmethod
    def stg_and_dtb_add_any_geometry_var_fnc(cls, dtb_opt, pattern_kwargs, pattern_opt_src, resource_dtb_path, version_dtb_path, file_path_src, var_index, file_format, keyword, is_var, is_lod):
        pattern_opt_tgt = dtb_opt.get_pattern_opt(keyword)
        #
        pattern_kwargs_src = copy.copy(pattern_kwargs)
        variants_src = pattern_opt_src.get_variants(file_path_src)
