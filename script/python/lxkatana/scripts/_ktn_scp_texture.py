...
class ScpTextureBuildCommand(object):
    ...
    def _create_shader(cls, data, extend_kwargs=None, to_view_center=False):
        ...
        #
        ktn_obj, is_create = ktn_core.NGObjOpt._generate_shader_create_args(path, type_name, shader_type_name)
    ...
    def __init__(self, texture_name, texture_assign):
        self._cfg = ctt_core.Content(
            value=bsc_core.ResourceConfigure.get_yaml('katana/node-graph/texture')
        )
        self._cfg.set(
    ...
    def create_material_group_and_material(self, to_view_center=False):
        ...
        mtl_grp_ktn_obj, is_create = ktn_core.NGObjOpt._generate_node_create_args(mtl_grp_path, mtl_grp_type_name)
        if is_create is True:
    ...
    def create_node_backdrop(self):
        ...
        w, h = 320, 80
        #
        node_bdp_ktn_obj, is_create = ktn_core.NGObjOpt._generate_node_create_args(node_bdp_path, node_bdp_type_name)
        if is_create is True:
    ...
    def update_node_group(self, key):
        ...
        path = self._cfg.get('node.{}.path'.format(key))
        ktn_obj, is_create = ktn_core.NGObjOpt._generate_node_create_args(path, type_name)
        obj_opt = ktn_core.NGObjOpt(path)
        obj_opt = ktn_core.NGObjOpt(ktn_obj)
        obj_opt.set_color((.25, .25, .5))
        #
        obj_opt.create_input_ports_by_data(self._cfg.get('node.{}.input_ports'.format(key)) or [])
...
ktn_group_opt = ktn_core.NGObjOpt('ShadingGroup')
texture_name = 'texture_name'
ktn_scripts.ScpTextureBuildForCreate(
    ...
    def __init__(self, node_arg, texture_name, texture_assign):
        if isinstance(node_arg, six.string_types):
            self._obj_opt = ktn_core.NGObjOpt(node_arg)
        else:
    ...
    def accept(self):
        def post_fnc_():
            if layout_path is not None:
                ktn_core.NGObjOpt(layout_path).gui_layout_shader_graph(
            ...
            if layout_path is not None:
                ...
                if drop_paths:
                    ktn_core.GuiNodeGraphOpt.drop_nodes(
                        map(lambda x: ktn_core.NGObjOpt(x).ktn_obj, drop_paths)
                    )
