...
from lxmaya.dcc.dcc_objects import _mya_dcc_obj_obj, _mya_dcc_obj_dag, _mya_dcc_obj_scene

import lxutil.dcc.dcc_objects as utl_dcc_objects


class GrowMesh(_mya_dcc_obj_dag.Shape):
    ...
    def get_painter_file_nodes(self):
        ...
        for path in self.get_painter_file_node_paths():
            file_reference_node = _mya_dcc_obj_obj.TextureReference(path)
            lis.append(file_reference_node)
        return lis
...
class Description(XGenObj):
    ...
    def get_grow_mesh_painter_file_nodes(self):
        ...
        for g in self.get_grow_meshes():
            [lis.append(i) for i in g.get_painter_file_nodes()]
        return lis

    def get_grow_mesh_painter_file_paths(self):
        return list(set([i.get_file().file_path for i in self.get_grow_mesh_painter_file_nodes()]))
...
class Palette(XGenObj):
    ...
    def get_file_path(self):
        directory_path = _mya_dcc_obj_scene.Scene.get_current_directory_path()
    ...
    def get_description_file_paths(self):
        ...
        for d in self.get_descriptions():
            self._get_files_in_node(lis, d)
            for o in d.get_objects():
                self._get_files_in_node(lis, o)
            for m in d.get_modules():
                self._get_files_in_node(lis, m)
        return lis

    def get_description_files(self):
        return [utl_dcc_objects.OsFile(i) for i in self.get_description_file_paths()]
...
class Port(object):
    ...
    def get_is_file_enable(self):
        ...
        node = self.node
        node_type = node.type
        if node_type in ma_configure.XGen.PATH_IGNORE_DICT:
            port_path = self.name
            if port_path in ma_configure.XGen.PATH_IGNORE_DICT[node_type]:
                ignore_config = ma_configure.XGen.PATH_IGNORE_DICT[node_type][port_path]
            ...
            if '''=map('${DESC}/''' in s:
                ...
                if re_results:
                    for re_result in re_results:
                        ...
                        self._get_file_in_path(lis, dir_path)
            # single files
