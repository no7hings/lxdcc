...
class FncLookAssImporterNew(utl_fnc_obj_abs.AbsFncOptionBase):
    PLUG_NAME = 'mtoa'
    #
    OPTION = dict(
        file='',
    ...
    def execute(self):
        ...
        if self.get('name_join_time_tag') is True:
            time_tag = bsc_core.TimestampOpt(bsc_core.StgFileOpt(file_path).get_modify_timestamp()).get_as_tag_36()
        else:
            time_tag = None
        #
