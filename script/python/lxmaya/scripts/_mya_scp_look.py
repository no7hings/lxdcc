...
class ScpLibraryLook(object):
    ...
    def __init__(self, location):
        ...
        self._group = mya_dcc_objects.Group(
            bsc_core.DccPathDagOpt(self._location).translate_to('|').get_path()
        )
...
mya_scripts.ScpLibraryLook.create_user_datas()
    ...
    def get_look_preview_data(self, texture_directory_path, resource_name):
        ...
        def bake_texture_fnc(shader_, texture_type_, key_, seq_):
            ...
            if bsc_core.StgPathMtd.get_is_exists(_texture_file_path) is False:
        ...
        def get_texture_file_path_fnc_(shader_, texture_type_, key_, seq_):
            ...
            if bsc_core.StgPathMtd.get_is_exists(_texture_file_path) is False:
                ...
                if _texture_src:
                    ...
                    if _result:
                        bsc_core.StgFileOpt(_result).set_copy_to_file(_texture_file_path)
            return _texture_file_path
        # noinspection PyUnresolvedReferences
        import maya.cmds as cmds

        s = bsc_core.StgFileSearchOpt(ignore_name_case=True, ignore_ext_case=True, ignore_ext=True)
    ...
    def collection_texture_fnc(s, shader, category_name, type_name, texture_type, key):
        ...
        if texture_src:
            ...
            if result:
                result_tgt = '{}/{}'.format(directory_path, bsc_core.StgFileOpt(result).get_name())
                result_tgt_opt = bsc_core.StgFileOpt(result_tgt)
                if result_tgt_opt.get_is_exists() is False:
                    bsc_core.StgFileOpt(result).set_copy_to_file(result_tgt)
            ...
            if i_surface_shader:
                for i_key, (i_mtd, i_atr) in texture_method_dict.items():
                    ...
                    if i_state is True:
                        ...
                        j = bsc_core.StgFileOpt('/production/library/resource/.data/3d_plant_proxy/texture-lost.json')
                        d_p = j.set_read()
                        d_p[i_file_path] = ''
                        j.set_write(d_p)
        return c.get_value()
...
mya_scripts.ScpLibraryLook('/geometries').split_meshes_by_subsets()
    ...
    def auto_group_by_component(self, category_path, resource_path):
        component_yml_file_opt = bsc_core.StgFileOpt(
            '/production/library/resource/.data/3d_plant_proxy/texture-component-mapper.yml'
        )
        search_json_file_opt = bsc_core.StgFileOpt(
        ...
        if data_1_:
            ...
            group = mya_dcc_objects.Group(bsc_core.DccPathDagOpt(location).translate_to('|').get_path())
