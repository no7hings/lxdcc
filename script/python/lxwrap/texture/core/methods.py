...
class TxrMethodForBuild(_AbsTxrMethod):
    CONTENT = None
    ...
    def _generate_content(cls):
        if cls.CONTENT is not None:
            return cls.CONTENT
        cls.CONTENT = rsc_core.ResourceConfigure.get_as_content('texture/build')
    ...
    def _generate_texture_data(directory_path, p, variants, mapper):
        ...
        for i_type, i_v in mapper.items():
            for j_keyword in i_v:
                ...
                if bsc_core.StgFileMtdForMultiply.get_is_exists(j_file_path) is True:
    ...
    def _generate_match_args(cls, file_opt):
        for i_p in cls.NAME_PATTERNS:
            ...
            i_number_args = bsc_core.StgFileMtdForMultiply.get_number_args(file_opt.get_name(), i_p_0)
            if i_number_args:
                i_file_name, _ = i_number_args
                return i_file_name, i_p
        return file_opt.get_name(), cls.NAME_PATTERNS[-1]

    def get_texture_args(self, file_path):
...
class TxrMethodForColorSpaceAsAces(_AbsTxrMethod):
    CONTENT = None
    INSTANCE = None
    ...
    def _generate_content(cls):
        if cls.CONTENT is not None:
            return cls.CONTENT
        cls.CONTENT = rsc_core.ResourceConfigure.get_as_content('texture/color-space/aces')
        cls.CONTENT.do_flatten()
        return cls.CONTENT
...
class TxrMethodForColorSpaceAsTxConvert(_AbsTxrMethod):
    CONTENT = None
    INSTANCE = None
    ...
    def _generate_content(cls):
        if cls.CONTENT is not None:
            return cls.CONTENT
        cls.CONTENT = rsc_core.ResourceConfigure.get_as_content('texture/color-space/convert')
        cls.CONTENT.do_flatten()
        return cls.CONTENT
